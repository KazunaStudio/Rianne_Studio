import React,{Component} from 'react';
import { Link } from 'react-router-dom';
import '../General.css';
import work1 from "../../Images/HearMe/HearMe.png";
import work2 from "../../Images/FYPJ/specs.png";
import work3 from "../../Images/PEEL/PeelLogo.png";
import work4 from "../../Images/PremiumApp/premiumapp.png";



export class Portfolio extends Component{
    render(){
        return(
            <div style={{paddingBottom:"15%", paddingTop:"3%"}}>
                <div className="with-lines" style={{marginBottom:"30px"}}>
                    <h4>Collection of my works</h4>
                    </div>
                <div className="bestworks spacing">
                    <div className="bestworksitem fadein">
                        <Link to="/HearingAidApp" className='navstyle'>
                            <div className='image1container'>
                                <img src={work1} className='imagework' alt="Logo"/>
                            </div>
                            <div className='textleft'>       
                                <b>Hearing Aid App</b> <br/> <span style={{fontFamily:"Ralewaylight",fontSize:"16px"}}>Hearing Aid App <br/> (UX Design)</span> 
                            </div>
                        </Link>
                    </div>
                    <div className="bestworksitem fadein">
                        <Link to="/SpectacleE-commerceApp" className='navstyle'>
                            <div className='image1container'>
                                <img src={work2} className='imagework' alt="Logo"/>
                            </div>
                            <div className='textleft'>
                                <b>Spectacle E-commerce App</b> <br/> <span style={{fontFamily:"Ralewaylight",fontSize:"16px"}}>First phase of the app creation <br/>(UX Design & First stage Development)</span> 
                            </div>
                        </Link>
                    </div>
                    <div className="bestworksitem fadein">
                        <Link to="/FnBWebsite" className='navstyle'>
                            <div className='image1container'>
                                <img src={work3} className='imagework' alt="Logo"/>
                            </div>
                            <div className='textleft'>
                                <b>FnB Website</b> <br/> <span style={{fontFamily:"Ralewaylight",fontSize:"16px"}}>Front-End of the app <br/> (UX Design & Partial Development) </span> 
                            </div>
                        </Link>
                    </div>
                </div>
                
                <div className="bestworks spacing" >
                    <div className="bestworksitem fadein">
                        <Link to="/PremiumApp" className='navstyle'>
                            <div className='image1container'>
                                <img src={work4} className='imagework roundimg' alt="Logo"/>
                            </div>
                            <div className='textleft'>       
                                <b>Premium App</b> <br/> <span style={{fontFamily:"Ralewaylight",fontSize:"16px"}}>Application <br/> (Full Stack Development)</span> 
                            </div>
                        </Link>
                    </div>
                    <div className="bestworksitem fadein hide">
                        <Link to="/" className='navstyle'>
                            <div className='image1container'>
                                <img src={work2} className='imagework' alt="Logo"/>
                            </div>
                            <div className='textleft'>
                                <b>In Progress</b> <br/> 
                                <span style={{fontFamily:"Ralewaylight",fontSize:"16px"}}>In Progress <br/>
                                In Progress</span> 
                            </div>
                        </Link>
                    </div>
                    <div className="bestworksitem fadein hide">
                        <Link to="/" className='navstyle'>
                            <div className='image1container'>
                                <img src={work4} className='imagework' alt="Logo"/>
                            </div>
                            <div className='textleft'>
                                <b>In Progress</b> <br/> 
                                <span style={{fontFamily:"Ralewaylight",fontSize:"16px"}}>In Progress
                                <br/>In Progress</span> 
                            </div>
                        </Link>
                    </div>
                </div>
            </div>
        )
    }
}